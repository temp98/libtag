...
ClickablePopup::ClickablePopup(NodePath* camera) : PandaNode("popup"), EventReceiver(), m_mouse_watcher(NametagGlobals::mouse_watcher),
                                                   m_cam(camera), m_disabled(false), m_clicked(false), m_hovered(false),
                                                   m_click_state(CLICKSTATE_NORMAL), m_click_event("")
{
...
void ClickablePopup::update_click_state()
{
    ...
    if (old_state == CLICKSTATE_NORMAL && state == CLICKSTATE_HOVER)
        NametagGlobals::rollover_sound->play();
        
    else if (state == CLICKSTATE_CLICK)
        NametagGlobals::click_sound->play();
